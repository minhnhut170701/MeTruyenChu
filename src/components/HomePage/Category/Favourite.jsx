...
import { getDataYT } from '../../../features/NovelTop/NovelTopSlice'
const Favourite = () => {
  const dispatch = useDispatch()
  const {dataYT} = useSelector((state) => state.novelTop)
  useEffect(() =>{
    dispatch(getDataYT())
  ...
  return (
    <Box mr="10px" w='30%'>
      <Text fontSize='18px' fontWeight='semibold' borderBottom='1px'>Yêu Thích</Text>
        <Box mt='20px'>
          <Flex alignItems='center'>
            ...
            {dataYT.slice(0,9).map((item, index) =>(
