...
import { getDataDC } from '../../../features/NovelTop/NovelTopSlice'

const TopNovel = () => {
  
  const dispatch = useDispatch()
  const {dataDC} = useSelector((state) => state.novelTop)
  useEffect(() =>{
    dispatch(getDataDC())
  ...
  return (
    <Box mr="10px" w='30%'>
        <Text fontSize='18px' fontWeight='semibold' borderBottom='1px'>Đề Cử</Text>
        <Box mt='20px'>
          <Flex alignItems='center'>
            ...
            {dataDC.slice(0,9).map((item, index) =>(
